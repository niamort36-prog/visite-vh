...
/** Un pylône de référence : position exacte et numéro officiel. */
  ...
  for (const r of lignes.slice(1)) {
    ...
    if (!liaisons[code]) {
      liaisons[code] = {
        ...
        nom: (r[iNom] ?? '').trim(),
        cm: (iCm >= 0 ? r[iCm] : '')?.trim() ?? '',
        gmr: (iGmr >= 0 ? r[iGmr] : '')?.trim() ?? '',
        eel: (iEel >= 0 ? r[iEel] : '')?.trim() ?? '',
      };
    }
    ancres.push({
      c: code,
...
export function resumeReferentiel(r: ReferentielRte) {
  ...
  for (const l of Object.values(r.liaisons)) {
    if (l.cm) cm.add(l.cm);
    if (l.gmr) gmr.add(l.gmr);
  }
  return {
